...
var mayTheChickenOvo: bool
var last
var stamina = 100
var modRED = 1*(100-stamina)/100.0
var modGREEN = 1*(stamina)/100.0
var t_passed = 0
...
func _ready():
	scene = get_tree().root.get_node("Game")
...
func tp_to_ovo():
	...
	sprite.modulate = Color8(255,255,255,80)
	position = Vector3(ovo.position.x, position.y, ovo.position.z)
	stamina = 0
	ovo.queue_free()
...
func _input(event):
	...
	if Input.is_action_just_pressed("drop_ovo") and not mayTheChickenOvo and stamina == 100 and ovo != null:
		tp_to_ovo()
